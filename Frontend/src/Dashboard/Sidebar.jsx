...
export default function Sidebar({ onSelect, sidebarOpen, setSidebarOpen }) {
  ...
  const handleSelect = (key) => {
    onSelect(key);
    setSidebarOpen(false); // Close sidebar after selecting an item
  };
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 bg-black bg-opacity-50 z-10 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}
        onClick={() => setSidebarOpen(false)}
      ></div>

      <aside
        className={`w-64 bg-[#838beb] text-white p-6 flex flex-col items-center rounded-r-3xl 
      ...
      >
        ...
        <div className="space-y-3 w-full mt-4">
          {menuItems.map((item) => (
            <button
              key={item.key}
              onClick={() => handleSelect(item.key)} // trigger page change and close sidebar
              className="w-full text-left px-4 py-2 hover:bg-[#5762E4] hover:text-white rounded-md"
            >
              {item.label}
            </button>
          ))}
          <button className="w-full text-left px-4 py-2 hover:bg-[#5762E4] hover:text-white rounded-md" onClick={() => navigate("/journal")}>
            Home Page
          </button>
        </div>
